...
public:
  C_IO_Dir_Commit_Ops(CDir *d, version_t v, int pr, bufferlist &bl,
                      vector<dentry_key_t> &rm, vector<CDir::dentry_commit_items> &s,
                      vector<string> &_stale)
    : dir(d), version(v), op_prio(pr) {
    dfts.swap(bl);
    to_remove.swap(rm);
    to_set.swap(s);
    _stale_items.swap(_stale);
  }

  void finish(int r) override {
    dir->_omap_commit_ops(r, op_prio, version, dfts, to_remove, _stale_items,
                          to_set);
  }
...
private:
  ...
  vector<CDir::dentry_commit_items> to_set;
  vector<string> _stale_items;
};

void CDir::_omap_commit_ops(int r, int op_prio, version_t version, bufferlist &dfts,
                            vector<dentry_key_t> &to_remove, vector<string> &_stales,
                            vector<dentry_commit_items> &to_set)
{
  dout(10) << __func__ << dendl;
  ...
  for (auto &item : to_set) {
    ...
    if (item.is_remote) {
      ...
      // dirfragtree
      bl.append(dftstr + item.dft_off, item.dft_len);
...
void CDir::_omap_commit(int op_prio)
{
  ...
  vector<string> _stale_items;
  _stale_items.reserve(stale_items.size());

  vector<dentry_key_t> to_remove;
  to_remove.reserve(50);

  vector<dentry_commit_items> to_set;
  to_set.reserve(items.size() > 50 ? items.size() : 50);

  bufferlist dfts(CEPH_PAGE_SIZE);

  if (!stale_items.empty()) {
    for (const auto &p : stale_items)
      _stale_items.push_back(string(p));
    stale_items.clear();
  }
  ...
  auto write_one = [&](CDentry *dn) {
	...
	snaps && try_trim_snap_dentry(dn, *snaps)) {
      ...
      if (unlikely(to_remove.size() == to_remove.capacity()))
        to_remove.reserve(to_remove.size());
    ...
    if (dn->get_linkage()->is_null()) {
      ...
      if (unlikely(to_set.size() == to_set.capacity()))
        to_set.reserve(to_set.size());
      dentry_commit_items item;
      item.key = key;
      item.dft_off = dfts.length();

      // try to reserve new size if there has less
      // than 1/8 page space
      if (item.dft_off % CEPH_PAGE_SIZE > 7 * CEPH_PAGE_SIZE / 8)
        dfts.reserve(CEPH_PAGE_SIZE);

      _parse_dentry(dn, item, snaps, dfts);
      item.dft_len = dfts.length() - item.dft_off;
