...
void CDir::_omap_commit(int op_prio)
{
  ...
  auto c = new C_IO_Dir_Commit_Ops(this, get_version(), op_prio, is_new(),
				   std::move(dfts), std::move(to_remove),
				   std::move(to_set), std::move(stale_items));
  stale_items.clear();
