...
===============
  $ device=`sudo multipath -l | grep 'LIO-ORG,TCMU device' | awk '{print $1}'`
  > sudo dd if=/dev/random of=/tmp/iscsi_tmpfile bs=1 count=1K 2&> /dev/null
  > sudo dd if=/tmp/iscsi_tmpfile of=/dev/mapper/$device bs=1 count=1K 2&> /dev/null
  > sudo dd if=/dev/mapper/$device of=/tmp/iscsi_tmpfile1 bs=1 count=1K 2&> /dev/null
  > sudo diff /tmp/iscsi_tmpfile /tmp/iscsi_tmpfile1

Logout the targets
==================
  $ IP=`cat /etc/ceph/iscsi-gateway.cfg |grep 'trusted_ip_list' | awk -F'[, ]' '{print $3}'`
  > sudo iscsiadm -m node -T iqn.2003-01.com.redhat.iscsi-gw:ceph-gw -p $IP:3260 -u | grep 'successful' | awk '{print $3}'
  successful.
