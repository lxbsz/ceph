...
Inode* Client::insert_trace(MetaRequest *request, MetaSession *session)
{
  ...
  if (reply->head.is_dentry) {
    ...
    std::scoped_lock il{diri->inode_lock};
    update_dir_dist(diri.get(), &dst);  // dir stat info is attached to ..
...
{
  ceph_assert(ceph_mutex_is_locked_by_me(req->request_lock));
  ceph_assert(ceph_mutex_is_locked_by_me(client_lock));
...
void Client::_put_request(MetaRequest *request)
{
  ...
  if (request->get_nref() == 1) {
    ...
    /*
     * Once the nref == 1,it means the request has already
     * been removed from the mds_requests, will be no any
     * other new user, so we are safe to release the lock.
     *
     * Or in the put() when destructing the request it will
     * hold the lock and we will get the assert abort.
     */
    rl.unlock();
  }
			  ...
			  bool drop_cap_releases)
{
  ...
  std::unique_lock cl{client_lock, std::defer_lock};
  // make the request
...
{
  std::unique_lock fl{fh->fh_lock, std::adopt_lock};
...
void Client::update_snap_trace(const bufferlist& bl, SnapRealm **realm_ret, bool flush)
{
  ...
  for (map<SnapRealm*, SnapContext>::iterator q = dirty_realms.begin();
	...
	cl.unlock();
        std::scoped_lock il(in->inode_lock);
	queue_cap_snap(in.get(), q->second);
	cl.lock();
      }
    } else {
...
void Client::handle_caps(const MConstRef<MClientCaps>& m)
{
  ...
  if (auto it = in->caps.find(mds); it != in->caps.end()) {
    ...
    switch (m->get_op()) {
      ...
      case CEPH_CAP_OP_GRANT: return handle_cap_grant(session.get(), in.get(), &cap, m);
      case CEPH_CAP_OP_FLUSH_ACK: return handle_cap_flush_ack(session.get(), in.get(), &cap, m);
    }
  } else {
...
void Client::lock_fh_pos(Fh *f)
{
  ...
  if (f->pos_locked || !f->pos_waiters.empty()) {
    ...
    std::unique_lock l{f->fh_lock, std::adopt_lock};
...
{
  Inode *in = f->inode.get();
  std::unique_lock fl{f->fh_lock, std::adopt_lock};
  std::unique_lock il{in->inode_lock, std::adopt_lock};
		   ...
		   const UserPerm& perms)
{
  ...
  Inode *quota_root;
  {
    std::scoped_lock ril{root->inode_lock};
    ...
    quota_root = root->quota.is_enable() ? root : get_quota_root(root, perms);
  }
  cl.lock();
...
int Client::_getlk(Fh *fh, struct flock *fl, uint64_t owner)
{
  ...
  std::scoped_lock fhl(fh->fh_lock);
...
int Client::rmsnap(const char *relpath, const char *name, const UserPerm& perms, bool check_perms)
{
  ...
  std::scoped_lock sil{in->inode_lock};
  if (cct->_conf->client_permissions) {
		      ...
		      const UserPerm& perms)
{
  ...
  return _listxattr(in.get(), list, size, perms);
}
...
int Client::flistxattr(int fd, char *list, size_t size, const UserPerm& perms)
{
  ...
  return _listxattr(f->inode.get(), list, size, perms);
}
...
public:
  ...
  void finish(int r) override {
    ...
    client->_interrupt_filelock(req.get());
  }
};
