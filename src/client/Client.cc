...
Client::Client(Messenger *m, MonClient *mc, Objecter *objecter_)
  : Dispatcher(m->cct),
    ...
    mount_state(CLIENT_UNMOUNTED),
    initialize_state(CLIENT_NEW),
...
int Client::init()
{
  RWRef iref_reader(initialize_state, CLIENT_NEW);
...
void Client::_finish_init()
{
  ...
  RWRef iref_writer(initialize_state, CLIENT_INITIALIZED, false);
}
...
void Client::shutdown() 
{
   ...
   * "readers" to finish.
   */
  RWRef iref_writer(initialize_state, CLIENT_NEW, false);
...
{
  RWRef iref_reader(initialize_state, CLIENT_INITIALIZED);
  ceph_assert(iref_reader.is_state_satisfied());
...
void Client::check_caps(Inode *in, unsigned flags)
{
  ...
  RWRef mref_reader(mount_state, CLIENT_MOUNTING);

  int retain = wanted | used | CEPH_CAP_PIN;
  if (mref_reader.is_state_satisfied() && in->nlink > 0) {
    if (wanted) {
      retain |= CEPH_CAP_ANY;
    } else if (in->is_dir() &&
  ...
  for (auto &p : in->caps) {
    ...
    if (!revoking && !mref_reader.is_state_satisfied() && (cap_used == 0))
      goto ack;
...
int Client::_do_remount(bool retry_on_error)
{
  ...
  if (r == 0) {
    ...
    if (should_abort && mref_reader.is_state_satisfied()) {
		  ...
		  bool require_mds, const std::string &fs_name)
{
   ...
   * is done.
   */
  RWRef mref_writer(mount_state, CLIENT_MOUNTING, false);
...
void Client::_unmount(bool abort)
{
   ...
   * "readers" to finish.
   */
  RWRef mref_writer(mount_state, CLIENT_UNMOUNTING, false);
