...
void Client::delay_put_requests(bool wakeup)
{
  ...
  {
    std::scoped_lock dl(delay_rlock);
    release.swap(requests_delay_release);
  }
...
{
  std::scoped_lock dl(delay_rlock);
  requests_delay_release.push_back(request);
}
...
void Client::delay_put_inodes(bool wakeup)
{
  ...
  std::multimap<Inode*,int> release;
  {
    std::scoped_lock dl(delay_lock);
    release.swap(delay_release);
    delay_release.clear();
  }
...
void Client::put_inode(Inode *in, int n)
{
  ...
  std::scoped_lock dl(delay_lock);
  delay_release.emplace(std::piecewise_construct,
                        std::make_tuple(in),
                        std::make_tuple(n));
}
		   ...
		   InodeRef *inp)
{
  ...
  MetaRequestRef req = create_request(dir->snapid == CEPH_SNAPDIR ?
