			 ...
			 bufferlist *pdirbl)
{
  ...
  // make note
  cl.lock();
  // unregister_request() will put() it.
  ...
  /*
   * This is no need to put under the client_lock, there members
   * are atomic type.
   */
...
{
  bool req_locked = false;

  /*
   * We need the request_lock here to avoid possibly
   * double-free issue for the 'request' bellow.
   */
  if (!request->request_lock.is_locked_by_me()) {
    req_locked = true;
    request->request_lock.lock();
  }

  request->put();
  if (request->get_nref() == 0) {
    bool locked = false;
    int op = -1;
    if (request->success)
      op = request->get_op();
    ...
    request->request_lock.unlock();
    req_locked = false;
    delete request;

    /*
     * If this called from MetaRequestRef's destructor, the client_lock
...
{
  ceph_assert(!ceph_mutex_is_locked_by_me(client_lock));
  ceph_assert(ceph_mutex_is_locked_by_me(request->request_lock));
  ...
  /*
   * No need to set it under client_lock for the session,
   * since we have reference increased, and for the con
   * it will have its own lock
   */
...
void Client::handle_client_request_forward(const MConstRef<MClientRequestForward>& fwd)
{
  ...
  cl.unlock();
  std::scoped_lock rl{request->request_lock};
...
void Client::handle_client_reply(const MConstRef<MClientReply>& reply)
{
  ...
  if (is_safe) {
    ...
    request->item.remove_myself();
    unregister_request(request.get());
  }
  if (is_unmounting())
    mount_cond.notify_all();
...
void Client::resend_unsafe_requests(MetaSession *session)
{
  ...
  for (const auto &p : session->unsafe_requests) {
    MetaRequestRef req(p);
    cl.unlock();
    std::scoped_lock rl{req->request_lock};
    send_request(req.get(), session);
    cl.lock();
  ...
  for (auto &p : mds_requests) {
    ...
    if (req->mds == session->mds_num)
      send_request(req.get(), session, true);
    cl.lock();
  }
...
void Client::wait_unsafe_requests()
{
  ...
  for (const auto &p : mds_sessions) {
    ...
    if (!s.unsafe_requests.empty()) {
      ...
      req->get(); // increase the ref
      last_unsafe_reqs.push_back(req);
    }
  }

  for (auto &p : last_unsafe_reqs) {
    MetaRequest *req = p;
    client_lock.unlock();
    std::scoped_lock rl{req->request_lock};
    client_lock.lock();
    ...
    put_request(req); // decrease the ref increased above
  }
}
		  ...
		  bool require_mds, const std::string &fs_name)
{
  ...
  while (true) {
    MetaRequestRef req = new MetaRequest(this, CEPH_MDS_OP_GETATTR);
    req->set_filepath(fp);
    req->head.args.getattr.mask = CEPH_STAT_CAP_INODE_ALL;
    int res = make_request(req.get(), perms);
    if (res < 0) {
      if (res == -EACCES && root) {
		       ...
		       InodeRef *target, const UserPerm& perms)
{
  ...
  int r = make_request(req.get(), perms, target);
		    ...
		    const UserPerm& perms)
{
  ...
  if (dname == "..") {
    if (dir->dentries.empty()) {
      MetaRequestRef req = new MetaRequest(this, CEPH_MDS_OP_LOOKUPPARENT);
      filepath path(dir->ino);
      req->set_filepath(path);

      InodeRef tmptarget;
      int r = make_request(req.get(), perms, &tmptarget, NULL, rand() % mdsmap->get_num_in_mds());
...
force_request:
  MetaRequestRef req = new MetaRequest(this, CEPH_MDS_OP_SETATTR);
  ...
  int res = make_request(req.get(), perms, inp);
...
int Client::_readdir_get_frag(dir_result_t *dirp)
{
  ...
  bufferlist dirbl;
  int res = make_request(req.get(), dirp->perms, NULL, NULL, -1, &dirbl);
			...
			const UserPerm& perms)
{
  ...
  std::scoped_lock lock(client_lock);
  MetaRequestRef req = new MetaRequest(this, CEPH_MDS_OP_LOOKUPHASH);
  filepath path(ino);
  req->set_filepath(path);
  ...
  int r = make_request(req.get(), perms, NULL, NULL,
...
int Client::_lookup_ino(inodeno_t ino, const UserPerm& perms, Inode **inode)
{
  ...
  MetaRequestRef req = new MetaRequest(this, CEPH_MDS_OP_LOOKUPINO);
  filepath path(ino);
  req->set_filepath(path);

  int r = make_request(req.get(), perms, NULL, NULL, rand() % mdsmap->get_num_in_mds());
  if (r == 0 && inode != NULL) {
    vinodeno_t vino(ino, CEPH_NOSNAP);
...
int Client::_lookup_parent(Inode *ino, const UserPerm& perms, Inode **parent)
{
  ...
  InodeRef target;
  int r = make_request(req.get(), perms, &target, NULL, rand() % mdsmap->get_num_in_mds());
...
int Client::_lookup_name(Inode *ino, Inode *parent, const UserPerm& perms)
{
  ...
  MetaRequestRef req = new MetaRequest(this, CEPH_MDS_OP_LOOKUPNAME);
		  ...
		  const UserPerm& perms)
{
  ...
  if ((flags & O_TRUNC) == 0 && in->caps_issued_mask(want)) {
    ...
    MetaRequestRef req = new MetaRequest(this, CEPH_MDS_OP_OPEN);
    filepath path;
    in->make_nosnap_relative_path(path);
    req->set_filepath(path);
    ...
    req->set_inode(in);
    result = make_request(req.get(), perms);
...
int Client::_renew_caps(Inode *in)
{
  ...
  if (pperm != NULL)
    perms = *pperm;
  int ret = make_request(req.get(), perms);
  return ret;
}
			 ...
			 struct flock *fl, uint64_t owner, bool removing)
{
  ...
  if (sleep && switch_interrupt_cb) {
    ...
    ret = make_request(req.get(), fh->actor_perms, NULL, NULL, -1, &bl);
    // disable interrupt
...
int Client::_interrupt_filelock(MetaRequest *req)
{
  ...
  MetaRequestRef intr_req = new MetaRequest(this, CEPH_MDS_OP_SETFILELOCK);
  filepath path;
  in->make_nosnap_relative_path(path);
  ...
  std::scoped_lock cl(client_lock);
  return make_request(intr_req.get(), perms, NULL, NULL, -1);
}
			 ...
			 size_t size, int flags, const UserPerm& perms)
{
  ...
  MetaRequestRef req = new MetaRequest(this, CEPH_MDS_OP_SETXATTR);
  filepath path;
  in->make_nosnap_relative_path(path);
  req->set_filepath(path);
...
int Client::_removexattr(Inode *in, const char *name, const UserPerm& perms)
{
  ...
  MetaRequestRef req = new MetaRequest(this, CEPH_MDS_OP_RMXATTR);
  filepath path;
  in->make_nosnap_relative_path(path);
  req->set_filepath(path);
		   ...
		   const UserPerm& perms, InodeRef *inp)
{
  ...
  MetaRequestRef req = new MetaRequest(this, CEPH_MDS_OP_MKNOD);
  ...
  res = make_request(req.get(), perms, inp);
		    ...
		    const UserPerm& perms)
{
  ...
  MetaRequestRef req = new MetaRequest(this, CEPH_MDS_OP_CREATE);
  ...
  res = make_request(req.get(), perms, inp, created);
  if (res < 0) {
    goto reply_error;
  }
		   ...
		   InodeRef *inp)
{
  ...
  MetaRequestRef req = new MetaRequest(this, dir->snapid == CEPH_SNAPDIR ?
  ...
  res = make_request(req.get(), perm, inp);
		     ...
		     const UserPerm& perms, InodeRef *inp)
{
  ...
  MetaRequestRef req = new MetaRequest(this, CEPH_MDS_OP_SYMLINK);
...
int Client::_unlink(Inode *dir, const char *name, const UserPerm& perm)
{
  ...
  MetaRequestRef req = new MetaRequest(this, CEPH_MDS_OP_UNLINK);
...
int Client::_rmdir(Inode *dir, const char *name, const UserPerm& perms)
{
  ...
  res = make_request(req.get(), perms);
...
int Client::_rename(Inode *fromdir, const char *fromname, Inode *todir, const char *toname, const UserPerm& perm)
{
  ...
  res = make_request(req.get(), perm, &target);
...
int Client::_link(Inode *in, Inode *dir, const char *newname, const UserPerm& perm, InodeRef *inp)
{
  ...
  in->break_all_delegs();
  MetaRequestRef req = new MetaRequest(this, CEPH_MDS_OP_LINK);
...
{
  MetaRequestRef req = static_cast<MetaRequest*>(d);
  ldout(cct, 3) << __func__ << " tid " << req->get_tid() << dendl;
  tout(cct) << __func__ << " tid " << req->get_tid() << std::endl;
  interrupt_finisher.queue(new C_Client_RequestInterrupt(this, req.get()));
}
