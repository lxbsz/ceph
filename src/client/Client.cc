...
void Client::shutdown() 
{
  ...
  {
    std::lock_guard lock(timer_lock);
    timer.shutdown();
  }
...
void Client::trim_cache_for_reconnect(MetaSessionRef &s)
{
...
void Client::trim_dentry(Dentry *dn)
{
  ldout(cct, 15) << "trim_dentry unlinking dn " << dn->name 
		 ...
		 << dendl;
  if (dn->inode) {
    InodeRef diri = dn->dir->parent_inode;
...
void Client::_fragmap_remove_non_leaves(InodeRef &in)
{
...
InodeRef Client::add_update_inode(InodeStat *st, utime_t from,
				    ...
				    Dentry *old_dentry)
{
  ...
  if (!dn || !dn->inode) {
	...
	InodeRef old_diri = old_dentry->dir->parent_inode;
 ...
 */
void Client::update_dir_dist(InodeRef &in, DirStat *dst)
{
  // auth
...
void Client::clear_dir_complete_and_ordered(InodeRef &diri, bool complete)
{
 ...
 */
void Client::insert_readdir_results(MetaRequestRef &request, MetaSessionRef &session, InodeRef &diri) {
  ceph_assert(!ceph_mutex_is_locked_by_me(client_lock));
  ...
  if (!p.end()) {
    ...
    for (unsigned i=0; i<numdn; i++) {
      ...
      std::lock_guard l(client_lock);
      InodeRef in = add_update_inode(&ist, request->sent_stamp, session,
				   request->perms);
      Dentry *dn;
 ...
 */
Inode* Client::insert_trace(MetaRequestRef &request, MetaSessionRef &session)
{
  ceph_assert(!ceph_mutex_is_locked_by_me(client_lock));
...
mds_rank_t Client::choose_target_mds(MetaRequestRef &req, Inode** phash_diri)
{
...
void Client::dump_mds_requests(Formatter *f)
{
  ...
  for (map<ceph_tid_t, MetaRequest*>::iterator p = mds_requests.begin();
    ...
    MetaRequestRef request = p->second;

    cl.unlock();
    {
      std::lock_guard rl{request->request_lock};
      request->dump(f);
    }
    cl.lock();
    f->close_section();
...
int Client::verify_reply_trace(int r, MetaSessionRef &session,
    ...
    *ptarget = request->target;
	...
	if (d->dir) {
	  ...
	  ldout(cct, 10) << "make_request got traceless reply, looking up #"
			 ...
			 << " ino " << created_ino
			 << dendl;
	  r = _do_lookup(diri, d->name, request->regetattr_mask,
			 &target, perms);
	  client_lock.lock();
	} else {
			 ...
			 bufferlist *pdirbl)
{
  ...
  MetaRequestRef tmp_request(request);
  std::unique_lock rl{request->request_lock};

  request->set_tid(tid);
  request->op_stamp = ceph_clock_now(); // and timestamp
  ...
  while (1) {
    ...
    mds_rank_t mds = choose_target_mds(tmp_request, &hash_diri);
    ...
    // send request.
    send_request(tmp_request, session);
  ...
  if (!request->reply) {
    ...
    unregister_request(tmp_request);
    put_request(request);
    return r;
  }
  ...
  if (request->_put()) {
	...
	(op == CEPH_MDS_OP_RMDIR ||
	 ...
	 op == CEPH_MDS_OP_RMSNAP)) {
      _try_to_trim_inode(other_in, false);
    }
  }
}

int Client::encode_inode_release(Inode *in, MetaRequestRef &req,
...
void Client::encode_dentry_release(Dentry *dn, MetaRequestRef &req,
 ...
 */
void Client::encode_cap_releases(MetaRequestRef &req, mds_rank_t mds)
{
...
MetaSession *Client::_open_mds_session(mds_rank_t mds)
{
  ...
  MetaSessionRef session = &em.first->second;
...
void Client::_close_mds_session(MetaSessionRef &s)
{
...
void Client::_closed_mds_session(MetaSessionRef &s, int err, bool rejected)
{
...
void Client::send_request(MetaRequestRef &request, MetaSessionRef &session,
...
ref_t<MClientRequest> Client::build_client_request(MetaRequestRef &request)
{
...
void Client::handle_client_request_forward(const MConstRef<MClientRequestForward>& fwd)
{
  ...
  MetaRequestRef request = mds_requests[tid];
  cl.unlock();
  std::unique_lock rl{request->request_lock};
...
bool Client::is_dir_operation(MetaRequestRef &req)
{
  int op = req->get_op();
...
void Client::handle_client_reply(const MConstRef<MClientReply>& reply)
{
  ...
  if (!is_safe || !request->got_unsafe) {
    ...
    // wake for kick back
    cond.wait(rl, [tid, request, &cond, this] {
...
void Client::_handle_full_flag(int64_t pool)
{
       ...
       i != inode_map.end(); ++i)
  {
    ...
    if (inode->oset.dirty_or_tx
      ...
      objectcacher_lock.lock();
      objectcacher->purge_set(&inode->oset);
      objectcacher_lock.unlock();
...
void Client::send_reconnect(MetaSessionRef &session)
{
  mds_rank_t mds = session->mds_num;
...
void Client::kick_requests(MetaSessionRef &session)
{
  std::unique_lock cl{client_lock, std::adopt_lock};
  ...
  for (map<ceph_tid_t, MetaRequest*>::iterator p = mds_requests.begin();
    ...
    cl.unlock();
    std::lock_guard rl{req->request_lock};
...
void Client::resend_unsafe_requests(MetaSessionRef &session)
{
  std::unique_lock cl{client_lock, std::adopt_lock};
...
void Client::kick_requests_closed(MetaSessionRef &session)
{
  std::unique_lock cl{client_lock, std::adopt_lock};
...
void Client::got_mds_push(MetaSessionRef &s)
{
  s->seq++;
...
void Client::close_dir(Dir *dir)
{
   ...
   */
Dentry* Client::link(Dir *dir, const string& name, InodeRef &in, Dentry *dn)
{
  if (!dn) {
    // create a new Dentry
  ...
  if (in) {    // link to inode
    ...
    if (in->is_dir() && !in->dentries.empty()) {
      ...
      InodeRef old_diri = olddn->dir->parent_inode;
...
int Client::get_caps_used(Inode *in)
{
  ...
  std::lock_guard l(objectcacher_lock);
...
void Client::cap_delay_requeue(InodeRef &in)
{
...
void Client::send_cap(Inode *in, MetaSessionRef &session, Cap *cap,
 ...
 */
void Client::check_caps(InodeRef &in, unsigned flags)
{
  ceph_assert(!ceph_mutex_is_locked(client_lock));
...
void Client::send_flush_snap(Inode *in, MetaSessionRef &session,
...
void Client::flush_snaps(Inode *in)
{
  ...
  MetaSessionRef session = in->auth_cap->session;
...
void Client::wake_up_session_caps(MetaSessionRef &s, bool reconnect)
{
...
void Client::check_cap_issue(InodeRef &in, unsigned issued)
{
...
void Client::add_update_cap(Inode *in, MetaSessionRef &session, uint64_t cap_id,
			    ...
			    inodeno_t realm, int flags, const UserPerm& cap_perms)
{
  ...
  /*
   * The 'mds_num' won't change until 'session' gets released,
   * no need to lock the client_lock here.
   */
  mds_rank_t mds = session->mds_num;
...
void Client::remove_cap(Cap *cap, bool queue_release)
{
  ...
  auto &in = cap->inode;
  MetaSessionRef session = cap->session;
...
void Client::remove_session_caps(MetaSessionRef &s, int err)
{
...
void Client::trim_caps(MetaSessionRef &s, uint64_t max)
{
  mds_rank_t mds = s->mds_num;
...
void Client::force_session_readonly(MetaSessionRef &s)
{
  s->readonly = true;
...
void Client::adjust_session_flushing_caps(Inode *in, MetaSessionRef old_s,  MetaSessionRef new_s)
{
  ceph_assert(!ceph_mutex_is_locked_by_me(client_lock));
...
void Client::kick_flushing_caps(InodeRef &in, MetaSessionRef &session)
{
  ceph_assert(!ceph_mutex_is_locked_by_me(client_lock));
...
void Client::kick_flushing_caps(MetaSessionRef &session)
{
  ceph_assert(ceph_mutex_is_locked_by_me(client_lock));
...
void Client::early_kick_flushing_caps(MetaSessionRef &session)
{
  ceph_assert(ceph_mutex_is_locked_by_me(client_lock));
...
void Client::handle_caps(const MConstRef<MClientCaps>& m)
{
  ...
  switch (m->get_op()) {
    case CEPH_CAP_OP_EXPORT:
      handle_cap_export(session, in, m);
      goto lock;
    case CEPH_CAP_OP_FLUSHSNAP_ACK:
      handle_cap_flushsnap_ack(session, in, m);
      goto lock;
    case CEPH_CAP_OP_IMPORT: /* no return */
  ...
  if (auto it = in->caps.find(mds); it != in->caps.end()) {
    ...
    switch (m->get_op()) {
      case CEPH_CAP_OP_TRUNC:
        handle_cap_trunc(session, in, m);
        goto lock;
      case CEPH_CAP_OP_IMPORT:
      case CEPH_CAP_OP_REVOKE:
      case CEPH_CAP_OP_GRANT:
        handle_cap_grant(session, in, &cap, m);
        goto lock;
      case CEPH_CAP_OP_FLUSH_ACK:
        handle_cap_flush_ack(session, in, &cap, m);
        goto lock;
    }
  } else {
...
void Client::handle_cap_import(MetaSessionRef &session, InodeRef &in, const MConstRef<MClientCaps>& m)
{
  ceph_assert(!ceph_mutex_is_locked_by_me(client_lock));
...
void Client::handle_cap_export(MetaSessionRef &session, InodeRef in, const MConstRef<MClientCaps>& m)
{
  ceph_assert(!ceph_mutex_is_locked_by_me(client_lock));
...
void Client::handle_cap_trunc(MetaSessionRef &session, InodeRef &in, const MConstRef<MClientCaps>& m)
{
  mds_rank_t mds = session->mds_num;
...
void Client::handle_cap_flush_ack(MetaSessionRef &session, InodeRef &in, Cap *cap, const MConstRef<MClientCaps>& m)
{
...
void Client::handle_cap_flushsnap_ack(MetaSessionRef &session, InodeRef &in, const MConstRef<MClientCaps>& m)
{
  ceph_assert(!ceph_mutex_is_locked_by_me(client_lock));
...
void Client::_try_to_trim_inode(InodeRef &in, bool sched_inval)
{
  ...
  if (in->dir && !in->dir->dentries.empty()) {
	...
	_try_to_trim_inode(dn->inode, false);
  ...
  if (ref > 0 && (in->flags & I_SNAPDIR_OPEN)) {
    InodeRef snapdir = open_snapdir(in);
    _try_to_trim_inode(snapdir, false);
    --ref;
  }
...
void Client::handle_cap_grant(MetaSessionRef &session, InodeRef &in, Cap *cap, const MConstRef<MClientCaps>& m)
{
  mds_rank_t mds = session->mds_num;
  int used = get_caps_used(in);
  int wanted = in->caps_wanted();
		  ...
		  bool require_mds, const std::string &fs_name)
{
  ...
  while (true) {
    MetaRequest *req = new MetaRequest(this, CEPH_MDS_OP_GETATTR);
    req->set_filepath(fp);
  ...
  while (!mds_sessions.empty()) {
    ...
    if (!timo) {
      ...
      while (!mds_ranks_closing.empty()) {
        MetaSessionRef s = &mds_sessions.at(*mds_ranks_closing.begin());
        // this prunes entry from mds_sessions and mds_ranks_closing
        _closed_mds_session(s, -ETIMEDOUT);
      }
    }
...
void Client::flush_mdlog(MetaSessionRef &session)
{
...
void Client::tick()
{
  ...
  if (!mounted && !mds_requests.empty()) {
    MetaRequestRef req = mds_requests.begin()->second;
    cl.unlock();
    std::lock_guard rl{req->request_lock};
...
void Client::renew_caps(MetaSessionRef &session)
{
		       ...
		       InodeRef *target, const UserPerm& perms)
{
  ...
  MetaRequest *req = new MetaRequest(this, op);
  filepath path;
		    ...
		    const UserPerm& perms)
{
  ...
  if (dname == "..") {
    if (dir->dentries.empty()) {
      MetaRequest *req = new MetaRequest(this, CEPH_MDS_OP_LOOKUPPARENT);
      filepath path(dir->ino);
      req->set_filepath(path);

      {
        std::scoped_lock cl(client_lock);
        mds_rank_t r_mds = rand() % mdsmap->get_num_in_mds();
      }

      InodeRef tmptarget;
      int r = make_request(req, perms, &tmptarget, NULL, r_mds);

      if (r == 0) {
	InodeRef tempino = tmptarget.get();
        std::scoped_lock il(tempino->inode_lock);
	_ll_get(tempino);
	*target = tempino;
  ...
  if (dir->dir &&
    ...
    /*
     * We are accessing the dentry inode's members, so we need to
     * take the dentry inode's inode_lock first.
     *
     * Be careful, we have alreayd token the dir inode's inode_lock
     * before this.
     */
    if (dn->inode) {
      dn->inode->inode_lock.lock();
    }
	...
	MetaSessionRef s = &mds_sessions.at(dn->lease_mds);
	if (s->cap_ttl > now &&
		      ...
		      const UserPerm& perms, bool followsym, int mask)
{
  ...
  {
    std::scoped_lock cl(client_lock);
    tmp_root = root; // increase ref to get rid of the client_lock in the while loop.
  ...
  while (i < path.depth() && cur) {
    ...
    // 'directory' symlinks.
    if (next) {
      std::scoped_lock il(next->inode_lock);
      if (next->is_symlink()) {
        symlinks++;
...
force_request:
  MetaRequest *req = new MetaRequest(this, CEPH_MDS_OP_SETATTR);
			...
			const UserPerm& perms)
{
  ...
  MetaRequest *req = new MetaRequest(this, CEPH_MDS_OP_LOOKUPHASH);
  filepath path(ino);
  req->set_filepath(path);
...
int Client::_lookup_ino(inodeno_t ino, const UserPerm& perms, Inode **inode)
{
  ...
  MetaRequest *req = new MetaRequest(this, CEPH_MDS_OP_LOOKUPINO);
  filepath path(ino);
  req->set_filepath(path);
...
int Client::_lookup_name(Inode *ino, Inode *parent, const UserPerm& perms)
{
  ...
  MetaRequest *req = new MetaRequest(this, CEPH_MDS_OP_LOOKUPNAME);
		  ...
		  const UserPerm& perms)
{
  ...
  if ((flags & O_TRUNC) == 0 && in->caps_issued_mask(want)) {
    ...
    MetaRequest *req = new MetaRequest(this, CEPH_MDS_OP_OPEN);
    filepath path;
    in->make_nosnap_relative_path(path);
    req->set_filepath(path);
...
int Client::_interrupt_filelock(MetaRequest *req)
{
  ...
  MetaRequest *intr_req = new MetaRequest(this, CEPH_MDS_OP_SETFILELOCK);
  filepath path;
  in->make_nosnap_relative_path(path);
			 ...
			 size_t size, int flags, const UserPerm& perms)
{
  ...
  MetaRequest *req = new MetaRequest(this, CEPH_MDS_OP_SETXATTR);
  filepath path;
  in->make_nosnap_relative_path(path);
  req->set_filepath(path);
...
int Client::_removexattr(Inode *in, const char *name, const UserPerm& perms)
{
  ...
  MetaRequest *req = new MetaRequest(this, CEPH_MDS_OP_RMXATTR);
  filepath path;
  in->make_nosnap_relative_path(path);
  req->set_filepath(path);
		   ...
		   const UserPerm& perms, InodeRef *inp)
{
  ...
  MetaRequest *req = new MetaRequest(this, CEPH_MDS_OP_MKNOD);
		    ...
		    const UserPerm& perms)
{
  ...
  MetaRequest *req = new MetaRequest(this, CEPH_MDS_OP_CREATE);
		   ...
		   InodeRef *inp)
{
  ...
  MetaRequest *req = new MetaRequest(this, dir->snapid == CEPH_SNAPDIR ?
		     ...
		     const UserPerm& perms, InodeRef *inp)
{
  ...
  MetaRequest *req = new MetaRequest(this, CEPH_MDS_OP_SYMLINK);
...
int Client::_unlink(Inode *dir, const char *name, const UserPerm& perm)
{
  ...
  MetaRequest *req = new MetaRequest(this, CEPH_MDS_OP_UNLINK);
...
int Client::_link(Inode *in, Inode *dir, const char *newname, const UserPerm& perm, InodeRef *inp)
{
  ...
  MetaRequest *req = new MetaRequest(this, CEPH_MDS_OP_LINK);
