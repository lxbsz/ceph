...
class CephFSMount(object):
    ...
    def _setup_brx_and_nat(self):
        ...
        if not brx:
            ...
            self.client_remote.run(args=args, timeout=(5*60))
            args = ['sudo', 'ip', 'addr', 'add', '{0}/{1}'.format(ip, maks), 'brd', '{0}'.format(brd), 'dev', 'ceph-brx']
            self.client_remote.run(args=args, timeout=(5*60))
    ...
    def _setup_netns(self):
        ...
        self.client_remote.run(args=args, timeout=(5*60))
        args = ['sudo', 'ip', 'netns', 'set', '{0}'.format(self.netns_name), '{1}'.format(nsid)]
        self.client_remote.run(args=args, timeout=(5*60))
        ...
        self.client_remote.run(args=args, timeout=(5*60))
        args = ['sudo', 'ip', 'netns', 'exec', '{0}'.format(self.netns_name), 'ip', 'addr', 'add', '{1}/{2}'.format(ip, mask), 'brd', '{3}'.format(brd), 'dev', 'veth0']
        self.client_remote.run(args=args, timeout=(5*60))
        ...
        args = ['sudo', 'ip', 'netns', 'exec', '{0}'.format(self.netns_name), 'ip', 'route', 'add', 'default', 'via', '{1}'.format(brxip)]
        self.client_remote.run(args=args, timeout=(5*60))
    ...
    def _cleanup_brx_and_nat(self):
        ...
        self.client_remote.run(args=args, timeout=(5*60))
        args = ['sudo', 'iptables', '-t', 'nat', '-D', 'POSTROUTING', '-s', '{0}/{1}'.format(ip, mask), '-o', '{2}'.format(gw), '-j', 'MASQUERADE']
        self.client_remote.run(args=args, timeout=(5*60))
