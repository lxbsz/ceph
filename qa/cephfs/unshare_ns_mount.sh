...
# This is one helper for mounting the ceph-fuse client and kernel
# client by unsharing the network namespace. With the unshared
# network namespace, you can easily suspend or resume the virtual
# network interface to simulate the client node hard shutdown for
# some test cases.
#
...
NET_MASK_DEF=16
BRD_MASK_DEF=192.168.255.255

CEPH_BRX_IP=$CEPH_BRX_IP_DEF
NET_MASK=$NET_MASK_DEF
BRD_MASK=$BRD_MASK_DEF
...
function get_mountpoint() {
    ...
    if [ "0$mountpoint" == "0" ]; then
        echo "Mountpoint does not exist or not specified!"
        exit 1
    fi
}
...
function get_new_netns() {
	...
	echo "Oops, no network namespace name found!"
	exit 1
    fi
}
...
function ceph_umount() {
    ...
    nsid=`ip netns list-id | grep "$new_netns" | awk '{print $2}'`
    netns_brx=brx.$nsid
    ip link set $netns_brx down 2>/dev/null
    brctl delif $CEPH_BRX $netns_brx 2>/dev/null
    ip link delete $netns_brx 2>/dev/null
...
function get_brd_mask() {
    ...
    BRD_MASK=$first.$second_max.$third_max.$fourth_max
}
...
function check_valid_private_ip() {
    ...
    # private network class A 192.168.0.0 - 192.168.255.255
    ...
    # private network class C 10.0.0.0 - 10.255.255.255
    if [ "$first" == "10" -a $NET_MASK -ge 8 ]; then
	return
    fi

    echo "ERROR: invalid private ip address '$CEPH_BRX_IP/$NET_MASK'!"
    exit 1
}
...
function setup_bridge_and_nat() {
	...
	if [ "0$new_brxip" != "0" -a "$existing_brxip" != "$new_brxip" ]; then
            echo "Error: Conflict with the existing $CEPH_BRX ip '$existing_brxip', new '$new_brxip'!"
	    exit 1
	fi
    ...
    ip addr add $CEPH_BRX_IP/$NET_MASK brd $BRD_MASK dev $CEPH_BRX
...
function ceph_mount() {
    ...
    if [ "0$rc" != "0" ]; then
        echo "Network namespace "$new_netns" already exists!"
        exit 1
    fi
    ...
    get_new_ns_ip ns_ip
    if [ 0"$ns_ip" == "0" ]; then
	echo "There is no ip address could be used!"
	exit 1
    fi
    ...
    ip netns exec $new_netns ip addr add $ns_ip/$NET_MASK brd $BRD_MASK dev $ns_veth
